...
type DepsResult =
    ...
    member this.Edges() = this.Graph |> Graph.collectEdges
...
module internal DependencyResolution =
    ...
    let emptyList<'a> () = System.Collections.Generic.List<'a>()
    ...
    let rec cloneTrie (trie: TrieNode) : TrieNode =
        let children =
            ...
            let children =
                trie.Children
                |> Seq.map (fun (KeyValue (segment, child)) -> KeyValuePair(segment, cloneTrie child))
    ...
    let buildTrie (files: FileData[]) : TrieNode =
        ...
        let addFile (file: FileData) =
            file.Data.Tops |> Array.iter (addFileIdent file)

        // For every file add all its top-level modules/namespaces to the Trie
        files |> Array.iter addFile
    ...
    let rec searchInTrie (trie: TrieNode) (path: LongIdent) : TrieNode option =
        ...
        | segment :: rest ->
            ...
            | true, child -> searchInTrie child rest
            | false, _ -> None
    ...
    let detectFileDependencies (files: SourceFiles) : DepsResult =
        ...
        let filesWithModuleAbbreviations =
            ...
            |> Array.filter (fun n ->
                ...
                |> Array.exists (function
                    | Abbreviation.ModuleAbbreviation -> true
                    | _ -> false))
        ...
        let fsiFiles =
            ...
            |> Array.filter (fun f ->
                ...
                | ASTOrX.AST (ParsedInput.SigFile _) -> true
                | _ -> false)
                ...
                | false ->
                    ...
                    let rec walkDownAndMark (id: SimpleId) (node: TrieNode) =
                        ...
                        | id ->
                            ...
                            | true, child -> walkDownAndMark id[1..] child
