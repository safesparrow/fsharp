...
let codebases =
    [|
        {
            WorkDir = $@"{__SOURCE_DIRECTORY__}\.checkouts\fcs\src\compiler"
            Path = $@"{__SOURCE_DIRECTORY__}\FCS.args.txt"
            Limit = None
        }
        {
            WorkDir = $@"{__SOURCE_DIRECTORY__}\.checkouts\fcs\tests\FSharp.Compiler.ComponentTests"
...
let internal setupParsed config =
    ...
    workingDir
    |> Option.iter (fun dir -> Environment.CurrentDirectory <- replaceCodeRoot dir)
